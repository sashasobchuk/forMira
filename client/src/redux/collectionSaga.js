...
import {
    ...
    addOneCollection,
    DELETE_COLLECTION,
    DOWNLOAD_COLLECTIONS, DOWNLOAD_ONE_COLLECTION, EDIT_COLLECTION, editLocalData, removeLocalCollection,
    setCollections,
    UPLOAD_COLLECTION
...
function* addOneCollectionWorker(payload) {
    let data =  yield call(downloadCollectionApi,payload.id)
    yield put(addLocalCollection(data.data))
}
...
function* downloadCollections() {
    ...
    yield put(setCollections(data.data))
}
